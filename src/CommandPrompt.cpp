...
int main(int argc, char* argv[])
{
	...
	while(active)
	{
		...
		else if(command == "forward")
		{
			...
			if(client.optimise_stiffness(direction)) output.addString("Forward");
			else output.addString("C'era un problema...");
		}
		else if(command == "left")
		{
			std::vector<double> direction = {0,1,0,0,0,0};
			
			if(client.optimise_stiffness(direction)) output.addString("Left");
			else output.addString("C'era un problema...");
		}
		else if(command == "up")
		{
			std::vector<double> direction = {0,0,1,0,0,0};
			
			if(client.optimise_stiffness(direction)) output.addString("Up");
			else output.addString("C'era un problema...");
		}
		else
		{
