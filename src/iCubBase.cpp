...
Eigen::MatrixXd iCubBase::partial_derivative(const Eigen::MatrixXd &J, const unsigned int &jointNum)
{
	...
	else if(jointNum > n-1)
	{
		throw std::invalid_argument("[ERROR] [iCUB BASE] partial_derivative(): Cannot compute the partial derivative for joint " + std::to_string(jointNum) + " since the input Jacobian only has " + std::to_string(n) + " columns.");
	}
	
	Eigen::MatrixXd dJ(6,n); dJ.setZero();
